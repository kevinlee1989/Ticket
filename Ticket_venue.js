...
const saveDataToDynamoDB = async (eventBody) => {
    ...
    const { VIDValue, nameValue, locationValue, seatsValue } = requestBody;
    ...
    const param = {
      ...
      Item: {
        ...
        Seats: seatsValue   // DynamoDB의 파티션 키인 "PID"에 저장
      }
    };
  ...
  const deleteDataFromDynamoDB = async (eventBody) => {
    ...
    const param = {
      TableName: 'Venue',
      Key: {
        VID: VIDValue  // 삭제할 데이터의 파티션 키 (PID)
      }
    };
  ...
  const getDataFromDynamoDB = async (eventBody) => {
    ...
    const {  VID: VIDValue } = requestBody;
  
    const param = {
      TableName: 'Venue',
      Key: {
        VID: VIDValue  // DynamoDB의 파티션 키로 조회
      }
    };
  ...
  const updateDataInDynamoDB = async (eventBody) => {
    ...
    const param = {
        ...
        Item: {
            ...
            Name: nameValue,
            Location: locationValue,
            Seats: seatsValue   // DynamoDB의 파티션 키인 "PID" 에 업데이트
        }
      };
